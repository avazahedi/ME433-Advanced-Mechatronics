...
def webpage():
    ...
    time_diff = str(time.time()) # time in seconds since epoch
    time_diff = str(6)
    # time_diff = str(0)
    ...
    <p><button class="button" name="LED" id="LED" value="ON" type="submit">LED ON</button></a></p>
    <p><button class="button" name="LED" id="LED" value="OFF" type="submit">LED OFF</button></a></p>
    <h1>Click the stopwatch to see how long has elapsed since the last press: {time_diff} seconds</h1><br>
    <p><button class="button" name="Stopwatch" value="time" type="submit">Stopwatch</button></a></p>
    </body>
    <script>
    // LED buttons handler
    $(".button").click((el) => {{ $.post("/led", {{[el.target.name]: el.target.value}}) }});
    // Stopwatch button handler
    $(".button").click((el) => {{ $.get("/stopwatch", (data) => {{ $("#time_diff").text(data); }}); }});
...
# $(".button").click((el) => {{ $.post("/stopwatch", {{[el.target.name]: el.target.value}}), $.get("/stopwatch", (data) => {{$("#time_diff").text(data);}}); }});

# $(".button").click((el) => {{$.get("/stopwatch", (data) => {{
#         $("#time_diff").text(data);
#         }}; 
#     }});
# $.post("/stopwatch", {{[el.target.name]: el.target.value}});
...
# get time since last stopwatch press
@server.route("/stopwatch")
def stopwatch(request: HTTPRequest):
    print("stopwatch request")

    # get time difference
    time_diff = str(time.time())

    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
        response.send(time_diff)
